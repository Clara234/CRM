...
public class PanelUsuario extends JPanel implements Servicios2{
	...
	protected void filtroUsuario() {
		...
		try {
			...
			ResultSet rs = c.getRS(ConfigDir.getInstance().getProperty("filtroUsu"));
			while (rs.next()) {
	...
	public class gestorInsertar implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				PreparedStatement ps = c.getPS(ConfigDir.getInstance().getProperty("query13"));
	...
	public class gestorBorrar implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (resp == JOptionPane.YES_OPTION) {
				...
				try {
					ps = c1.getPS(ConfigDir.getInstance().getProperty("query14"));
	...
	public class gestorActualizar implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				PreparedStatement ps = new MisConexiones().getPS(ConfigDir.getInstance().getProperty("query16"));
				ps.setString(1, tf_alias.getText());
				ps.setString(2, tf_clave.getText());
				ps.setInt(3, Integer.valueOf(tf_grupo.getText()));
	...
	public void refresh() {
		...
		try {
			...
			c = new MisConexiones();
			listaUsuarios = new ArrayList<Usuario>();
			ResultSet rs = c.getRS(ConfigDir.getInstance().getProperty("query12"));
